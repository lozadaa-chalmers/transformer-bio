...
class TestScoreCategories(unittest.TestCase):
    ...
    cell_classification.score_categories(pre_processed_adata=adata,
    ...
    def test_c1_correct(self):
        with self.assertRaises(ValueError):
            self.assertEqual(self.adata.obs.c1, self.c1_test)

    def test_c2_correct(self):
        with self.assertRaises(ValueError):
            self.assertEqual(self.adata.obs.c2, self.c2_test)

    def test_c3_correct(self):
        with self.assertRaises(ValueError):
            self.assertEqual(self.adata.obs.c3, self.c3_test)
...
class TestAnnotateCategories(unittest.TestCase):
    ...
    cell_classification.score_categories(pre_processed_adata=adata,
                                         unique_categories_list=unique_categories_list,
                                         class_df=class_df)
    cell_classification.annotate_categories(scored_adata=adata,
                                            unique_categories_list=unique_categories_list)
    max_class_column = adata.obs[unique_categories_list].idxmax(axis=1)

    def test_classes_correct(self):
        with self.assertRaises(ValueError):
            self.assertEqual(self.adata.obs.suva_class, self.max_class_column)
